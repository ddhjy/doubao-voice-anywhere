...
/// 「输入法」分栏：日常输入源的两端，以及 Ctrl+Space 轮换开关。
        ...
        Form {
            Section {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text("语音结束后找不到之前的输入源时会恢复到这里，它们也是 Ctrl+Space 轮换的两端。「按 Fn 说话」固定使用豆包输入法，不需要配置。")
            ...
            Section {
                Toggle("用 Ctrl+Space 轮换输入源", isOn: store.ctrlSpaceSwitchEnabled)
            } footer: {
                Text("只在上面两个输入源之间切换，不把豆包放进轮换。关掉后 Ctrl+Space 交回系统处理。")
            }
        }
        .formStyle(.grouped)
        .settingsAlert(store)
