...
- App 在启动和唤醒后立即静默拉起豆包输入法；进程退出时及时重启，另用 5 秒巡检补漏。拉起失败会退避重试，异步拉起超时也能恢复；用户开始说话后，旧预热回调不能再切走输入法。后台保活不会触发语音录音。
- 左 Option 单击对豆包来说既是「开始」也是「结束」。每次启动都先刷新前台 App 的输入上下文，再发出第一次 Option，避免切换应用或闲置后首个按键落空。输入法准备与 200ms 的快捷键释放等待并行，切换稳定期也与焦点刷新重叠。
